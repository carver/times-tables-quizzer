...
Local to the browser by default — no backend, no login. Progress lives in `localStorage`. Cross-device sync (docs/adr/0006) is opt-in: turning it on is the only thing that talks to Firebase, and its SDK is only downloaded by a device that actually turns it on.
...
`src/cloudSync.ts` defaults to Firebase's `demo-times-tables-quizzer` project ID, which only ever talks to a local emulator (`firebase-tools`, already a dev dependency) - never a real cloud project, and needs no Google account to develop or test against:
...
To point the app at a real Firebase project instead (once one exists), copy `.env.example` to `.env` and fill in that project's config from the Firebase console.
...
- **[`CONTEXT.md`](CONTEXT.md)** — the glossary. Fact, Attempt, Fluency, Accuracy, Mastered, Active range, Streak, Celebration, Profile. Use these words; each entry lists the synonyms to avoid.
