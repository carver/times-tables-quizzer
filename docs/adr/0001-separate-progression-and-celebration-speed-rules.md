...
Amendment (2026-08-27): `TARGET_SPEED_MS` was raised from 2000ms to 2500ms because the clock starts when the prompt renders and also pays for reading, tapping, and Enter; see the constant's comment in `src/engine/engine.ts`. The rule itself (one fixed target for progression, a personal baseline for celebration) is unchanged.
